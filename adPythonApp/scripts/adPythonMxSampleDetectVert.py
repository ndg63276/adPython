...
def locate_sample_vert(edge_arr, params):
    # Same as locate_sample, but rotated 90 degrees
    ...
    for y in rows:
        # Look for the first non-narrow region between left and right edges.
        if tip_y is None and left[y] is not None \
        and abs(left[y] - right[y]) > min_tip_height:
            
            # Move backwards to where there were no edges at all...
            while left[y] is not None:
                y += -direction
                if y == -1 or y == height:
                    # (In this case the sample is off the edge of the picture.)
                    break
            y += direction # ...and forward one step. y is now at the tip.

            tip_y = y
            tip_x = int(round(0.5*(left[y] + right[y])))

            # Zero the edge arrays above (below) the tip.
            if direction == 1:
                left[:y] = [None for _ in xrange(y)]
                right[:y] = [None for _ in xrange(y)]
            else:
                assert direction == -1
                left[x:] = [None for _ in xrange(y)]
                right[x:] = [None for _ in xrange(y)]

    # Prepare for export to PVs.
    left = np.asarray(
        [none_value if l is None else l for l in left], dtype=np.int32)
    right = np.asarray(
        [none_value if r is None else r for r in right], dtype=np.int32)
    if tip_y is None or tip_x is None:
        tip_y, tip_x = -1, -1

    return (tip_y, tip_x), (left, right)
...
class MxSampleDetect(AdPythonPlugin):
    ...
    def processArray(self, arr, attr={}):
        ...
        tip, edges = locate_sample_vert(closed_arr, location_params)
